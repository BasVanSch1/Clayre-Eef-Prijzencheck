...
import { toggleEnabledUser } from "~/services/userService.server";

export const handle = {
  title: "Maintenance > Users > Toggle Enable User",
};
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const result = await toggleEnabledUser(params.id);
...
export default function ToggleEnabledUser() {
        ...
        {!code ? (
          ...
          <>
            <p className="text-red-600 text-lg">
              {code && message
                ? `Error ${code} : ${message}`
                : "An error occurred while trying to toggle the enabled status of the user."}
            </p>
